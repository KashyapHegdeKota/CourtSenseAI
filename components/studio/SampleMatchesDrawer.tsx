        ...
        <div className="grid gap-3 border-t border-white/5 px-5 py-4 md:grid-cols-3">
          {SAMPLE_MATCHES.map((sample) => {
            ...
            return <button key={sample.id} type="button" disabled={analyzing} onClick={() => onSelect(sample)} className={`group relative min-h-32 overflow-hidden rounded-2xl border p-4 text-left transition-all disabled:cursor-wait disabled:opacity-60 ${active ? accents[sample.accent] : "border-white/10 bg-white/[.025] hover:-translate-y-0.5 hover:border-white/20 hover:bg-white/[.045]"}`}>
              ...
              <span className="relative mt-6 block text-sm font-semibold text-slate-100">{sample.title}</span><span className="relative mt-1 block text-[11px] text-slate-500">{analyzing ? "Loading precomputed pitch tracking…" : sample.subtitle}</span>
            </button>;
          })}
        </div>
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-white/5 bg-black/10 px-5 py-3 text-[11px] text-slate-500"><span className="flex items-center gap-2"><Sparkles size={13} className="text-emerald-300"/> Selecting a demo loads its annotated stream and synchronized telemetry.</span>{error && <span className="text-orange-300">{error}</span>}</div>
